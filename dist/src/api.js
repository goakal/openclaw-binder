...
export async function postBinderMessage(params) {
    ...
    const messageId = await res
        .json()
        .then((data) => (typeof data?.message?.id === "string" ? data.message.id : null))
        .catch(() => null);
 ...
 */
export async function loadBinderMedia(source, readFile) {
    const isHttp = /^https?:\/\//i.test(source);
    let bytes;
    if (isHttp) {
        const res = await fetch(source);
        if (!res.ok)
            throw new Error(`Failed to fetch media ${res.status}: ${source}`);
...
export async function uploadBinderAttachment(params) {
    ...
    if (!groupId === !conversationId) {
        throw new Error("uploadBinderAttachment: provide exactly one of groupId / conversationId");
    }
    ...
    const { id, signedUrl } = (await initRes.json());
 ...
 */
export async function probeBinderToken(account) {
