...
const lastMessageIdByGroup = new Map();
export function setBinderLastMessageId(groupId, messageId) {
    lastMessageIdByGroup.set(groupId, messageId);
}
function getBinderLastMessageId(groupId) {
    return lastMessageIdByGroup.get(groupId);
}
const binderConfigBase = createScopedChannelConfigBase({
    sectionKey: "binder",
...
export const binderPlugin = {
    ...
    messaging: {
        ...
        targetResolver: {
            ...
            looksLikeId: (raw, normalized) => {
                ...
                return /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(check.replace(/^binder:/i, ""));
            },
        ...
        parseExplicitTarget: ({ raw }) => {
            ...
            if (!groupId)
                return null;
            return { to: groupId, chatType: "group" };
        },
    ...
    config: {
        ...binderConfigBase,
        isConfigured: (account) => Boolean(account.config.apiUrl?.trim() &&
    ...
    setup: {
        ...
        validateInput: ({ input }) => {
            const i = input;
            if (!i.apiUrl?.trim())
                return "Binder requires --api-url (e.g. https://binder.example.com)";
            if (!i.botId?.trim())
                return "Binder requires --bot-id";
            if (!i.token?.trim())
                return "Binder requires --token (bearer token)";
            if (!i.webhookSecret?.trim())
                return "Binder requires --webhook-secret (from bot creation response)";
            if (!i.botUsername?.trim())
                return "Binder requires --bot-username (the bot's @handle)";
            return null;
        },
        applyAccountConfig: ({ cfg, accountId, input }) => {
            const i = input;
            const section = cfg.channels?.["binder"] ?? {};
            const accounts = section?.["accounts"] ?? {};
            const effectiveAccountId = normalizeAccountId(accountId);
            const patch = {};
            if (i.apiUrl?.trim())
                patch["apiUrl"] = i.apiUrl.trim();
            if (i.botId?.trim())
                patch["botId"] = i.botId.trim();
            if (i.token?.trim())
                patch["token"] = i.token.trim();
            if (i.webhookSecret?.trim())
                patch["webhookSecret"] = i.webhookSecret.trim();
            if (i.botUsername?.trim())
                patch["botUsername"] = i.botUsername.trim();
            if (i.webhookPath?.trim())
                patch["webhookPath"] = i.webhookPath.trim();
            if (i.name?.trim())
                patch["name"] = i.name.trim();
            if (i.verbose !== undefined)
                patch["verbose"] = i.verbose;
            return {
                ...cfg,
                channels: {
                    ...cfg.channels,
    ...
    gateway: {
        startAccount: async (ctx) => {
            ...
            await runPassiveAccountLifecycle({
                ...
                start: async () => monitorBinderProvider({
                    account,
                    config: ctx.cfg,
                    runtime: ctx.runtime,
                    abortSignal: ctx.abortSignal,
